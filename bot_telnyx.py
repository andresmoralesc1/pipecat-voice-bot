...
# NOTE: settings validates all required API keys via model_validator
# which reads from *_FILE env vars pointing to secrets files
...
# Validate TELNYX_API_KEY (not handled by settings)
if not os.getenv("TELNYX_API_KEY"):
    logger.error("❌ Missing env var: TELNYX_API_KEY")
    sys.exit(1)
...
# ============================================================================
# TTS Wrapper - Limpia texto antes de enviar a Cartesia
# ============================================================================

class CleanedCartesiaTTS(CartesiaTTSService):
    """
    Wrapper de CartesiaTTSService que limpia el texto antes de sintetizar.

    Aplica reglas de limpieza para evitar problemas de pronunciación:
    - Expande códigos de reserva (ej: ALEJANDRO-7708 → ALEJANDRO 7 7 0 8)
    - Elimina signos de puntuación problemáticos
    """

    async def _generate_tts_input(self, text, *args, **kwargs):
        """Limpiar texto antes de generarlo."""
        cleaned = clean_tts_text(text)
        if cleaned != text:
            logger.debug(f"🧹 TTS cleaned: '{text[:40]}...' → '{cleaned[:40]}...'")
        return await super()._generate_tts_input(cleaned, *args, **kwargs)
...
async def handle_create_reservation(params: FunctionCallParams):
    ...
    # restaurant_id ya no se valida - el backend usa el UUID default
    is_valid, errors = validate_reservation_params(
        ...
        restaurant_id="",  # Vacío - el backend maneja el restaurante default
        ...
        available_restaurants=None,  # No validar - el backend decide
    )
    ...
    # No enviamos restaurantId - el backend usa el UUID default automáticamente
    api_params = {
...
async def handle_modify_reservation(params: FunctionCallParams):
    ...
    # Guard: reject empty changes before calling API
    changes = args.get("changes", {})
    if not changes or len(changes) == 0:
        metrics.function_called("modify_reservation", success=False)
        logger.warning(f"[modify_reservation] changes vacio para codigo {args.get('code', '?')}")
        await params.result_callback({
            "voice_message": "ERROR: No especificaste que cambiar. Pregunta al cliente que quiere modificar (fecha, hora o numero de personas) y vuelve a llamar con el campo correspondiente: newPartySize, newDate o newTime. NUNCA envies changes vacio.",
            "success": False,
        })
        return
...
async def handle_create_admin_alert(params: FunctionCallParams):
    """Handle create_admin_alert function call."""
    metrics.function_called("create_admin_alert")
    result = await call_reservations_api("createAdminAlert", params.arguments)
    await params.result_callback({
        "voice_message": result.get("voiceMessage", "Alerta creada"),
        "success": result.get("success", False),
    })
...
def build_tools():
    ...
    get_reservation_func = FunctionSchema(
        ...
        description="Get details of an existing reservation by code. Ask the customer for it naturally: 'tu nombre seguido de 4 numeros, lo tienes en el WhatsApp de confirmacion'.",
        properties={
    ...
    cancel_reservation_func = FunctionSchema(
        ...
        description="Cancel an existing reservation. Requires code and phone. Ask for the code naturally, never spell out the format.",
        properties={
    ...
    modify_reservation_func = FunctionSchema(
        ...
        description="Modify an existing reservation. IMPORTANT: the 'changes' object MUST contain at least one field. Examples: changes={newPartySize: 4} or changes={newDate: '2026-05-10'} or changes={newTime: '21:00'}. NEVER send changes={}.",
        properties={
            "code": {"type": "string", "description": "Reservation code provided by the customer"},
            "phone": {"type": "string", "description": "Phone number for verification (without +34 prefix)"},
            "changes": {
                "type": "object",
                "description": "MUST include at least one field. Use newPartySize for people changes, newDate for date changes (YYYY-MM-DD), newTime for time changes (HH:MM). Example: if client says '4 personas' -> {newPartySize: 4}. NEVER send empty {}.",
                "properties": {
                    ...
                    "newPartySize": {"type": "integer", "description": "New number of people. Client says '4 personas' -> newPartySize: 4"},
                    "newSpecialRequests": {"type": "string", "description": "New special requests or observations"},
                },
            },
        },
    ...
    create_admin_alert_func = FunctionSchema(
        name="create_admin_alert",
        description="Create an alert for the admin team when human intervention is needed. Call this BEFORE telling the client that the team will contact them.",
        properties={
            "alertType": {"type": "string", "enum": ["large_group", "human_requested", "bot_error", "private_event", "complaint", "same_day"], "description": "Type of alert"},
            "reason": {"type": "string", "description": "Brief description in Spanish of why human assistance is needed"},
            "customerPhone": {"type": "string", "description": "Customer phone number from caller ID"},
            "customerName": {"type": "string", "description": "Customer name if captured, empty string if not"},
        },
        required=["alertType", "reason", "customerPhone"],
    )
...
async def bot(runner_args: RunnerArguments):
    ...
    logger.info(f"🔑 TELNYX_API_KEY being used: {telnyx_api_key[:20]}...{telnyx_api_key[-10:]} (len={len(telnyx_api_key)})")
    ...
    logger.info(f"✅ Cartesia TTS initialized (8 kHz, {settings.DEFAULT_VOICE}) with text cleaning")
    ...
    llm.register_function("create_admin_alert", handle_create_admin_alert)
